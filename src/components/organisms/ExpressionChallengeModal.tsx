...
export function ExpressionChallengeModal({
  ...
  const options = useMemo(() => {
    ...
    return new (faceapi as any).TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.6 });
  }, [faceapi]);
  ...
  const loggedStart = useRef(false); // Tambahan untuk log sekali saja
  ...
  useEffect(() => {
    ...
    // log mulai challenge hanya sekali
    ...
    const start = async () => {
      try {
        ...
        yawHistory.current = []; blinkState.current = { closed: false };
        setFrames(0); setMatched(0); setBestScore(0); setCountdown(durationSec);
        startAt = performance.now();

        const tick = async () => {
          if (!videoRef.current || !faceapi || !options || !loadedMeta.current) {
          ...
          try {
            ...
            if (main) {
              ...
              const mouthOpen = Math.hypot(topInner.x - bottomInner.x, topInner.y - bottomInner.y) /
              ...
              if (type === "senyum") { score = happy; passFrame = score >= threshold; }
                ...
                if (!blinkState.current.closed && ear < 0.19) blinkState.current.closed = true;
                if (blinkState.current.closed && ear > 0.23) {
                  blinkState.current.closed = false; passFrame = true; score = 1;
                }
              }
                ...
                const minYaw = Math.min(...yawHistory.current),
                  maxYaw = Math.max(...yawHistory.current);
              ...
              if (now - (lastLog.current || 0) > 500) {
                onLog?.(`Frame#${framesRef.current} yaw=${pose.yaw.toFixed(1)} pitch=${pose.pitch.toFixed(1)} score=${score.toFixed(2)} ${passFrame ? "✓" : ""}`);
                lastLog.current = now;
              }
            }
    ...
    const stop = () => {
      ...
      loggedStart.current = false; // reset untuk challenge berikutnya
    };
